...
class DeclareResultUtils {
    ...
    async calculateWinningPoints(betType, betAmount, gameRate) {
        switch (betType) {
            ...
            case 'Jodi Digit':
                return betAmount * (gameRate.jodiDigit) / 10;
            case 'Single Pana':
            ...
            case 'Half Sangam':
                return betAmount * (gameRate.halfSangam) / 10;
            case 'Full Sangam':
                return betAmount * (gameRate.fullSangam) / 10;
            default:
    ...
    async createFormattedList(filteredData, gameRate) {
        ...
        const result = await Promise.all(filteredData.map(async (item) => {
            ...
            const bidNumber = item.betNumber.join(', ');

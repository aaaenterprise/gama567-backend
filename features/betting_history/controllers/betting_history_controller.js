...
class BettingHistoryController {
    ...
    async deleteBettingHistoryById(req, res) {
        try {
            const game = await BettingHistoryServices.deleteBettingHistoryById( req.params.id);
            res.status(httpStatusCodes.ok).json({ success: true, message: "Betting History deleted", data: game });
        } catch (error) {
            res.status(httpStatusCodes.internalServerError).json({ success: false, message: error.message, data: {} });
        }
    }
